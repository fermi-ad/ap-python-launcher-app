...
def test_apps_returns_list_with_apps(client, mock_harbor):
    mock_harbor._repos = [HarborRepo(repo="proj/app", tag="latest")]
...
def test_launch_missing_tag_defaults_to_latest(client, mock_launcher):
    ...
    assert mock_launcher.last_create_job_kwargs["tag"] == "latest"
...
def test_launch_success_returns_launch_id(client):
...
def test_launch_strips_http_from_image_ref(mocker, base_config):
    ...
    mocker.patch("ap_launcher.server.KubeLauncher", return_value=instance)
    mocker.patch("ap_launcher.server.HarborClient", return_value=FakeHarborClient())
...
def test_launch_constructs_image_from_config_host_and_repo(
    client, mock_launcher, base_config
):
...
def test_launch_limit_exceeded_returns_429(client, mock_launcher, mocker):
    mocker.patch.object(
        mock_launcher,
        "create_job",
...
def test_launch_value_error_returns_400(client, mock_launcher, mocker):
    mocker.patch.object(
...
def test_launch_access_field_is_pending(client):
    resp = client.post("/launch", json={"repo": "proj/app", "tag": "latest"})
    data = resp.json()
...
def test_launch_status_returns_valid_status(client):
    launch_id = client.post(
        "/launch", json={"repo": "proj/app", "tag": "latest"}
    ).json()["launchId"]
...
def test_launch_status_returns_correct_launch_id(client):
    launch_id = client.post(
        "/launch", json={"repo": "proj/app", "tag": "latest"}
    ).json()["launchId"]
    resp = client.get(f"/launch/{launch_id}")
