...
def test_list_repositories_single_page(mocker, client_no_auth):
    ...
    mocker.patch.object(client_no_auth.session, "get", return_value=_make_response(items))
...
def test_list_repositories_raises_on_non_list_response(mocker, client_no_auth):
    ...
    with pytest.raises(RuntimeError, match="Unexpected Harbor response for repositories"):
        client_no_auth.list_repositories()
...
def test_list_artifacts_raises_on_non_list_response(mocker, client_no_auth):
    mocker.patch.object(
        client_no_auth.session, "get",
        return_value=_make_response("not a list")
    )
...
def _artifacts_with_latest():
    return _make_response([{"tags": [{"name": "latest"}]}])
...
def test_list_latest_apps_returns_repos_with_latest_tag_only(mocker, client_no_auth):
    side_effects = [
        ...
        _artifacts_with_latest(),    # app-a has latest
