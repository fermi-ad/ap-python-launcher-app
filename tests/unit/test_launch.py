...
def test_service_name_basic_format(launcher):
    name = launcher._service_name_for_launch(repo="proj/my-app", launch_id="abcd1234-0000-0000-0000-000000000000")
    assert name.startswith("appl-my-app-abcd1234")


def test_service_name_truncated_to_63_chars(launcher):
    long_repo = "proj/" + "x" * 100
    name = launcher._service_name_for_launch(repo=long_repo, launch_id="abcd1234-0000-0000-0000-000000000000")
    assert len(name) <= 63
...
def test_service_name_underscores_and_dots_replaced_with_dashes(launcher):
    name = launcher._service_name_for_launch(repo="proj/my_app.v2", launch_id="abcd1234-0000-0000-0000-000000000000")
...
def test_parse_lb_annotations_valid_json(launcher):
    launcher.lb_annotations_json = '{"service.beta.kubernetes.io/aws-load-balancer-type": "nlb"}'
...
def test_count_active_jobs_with_repo_filter_includes_repo_in_label_selector(launcher, mock_batch_api):
    mock_batch_api.list_namespaced_job.return_value = _jobs_list()
...
def test_create_job_creates_loadbalancer_service(launcher, mock_batch_api, mock_core_api):
    _setup_create_job(launcher, mock_batch_api, mock_core_api)
    launcher.create_job(image="img:latest", repo="proj/app", tag="latest")
    mock_core_api.create_namespaced_service.assert_called_once()


def test_create_job_returns_launch_result_with_non_empty_id(launcher, mock_batch_api, mock_core_api):
    _setup_create_job(launcher, mock_batch_api, mock_core_api)
...
def test_create_job_job_name_truncated_to_63_chars(launcher, mock_batch_api, mock_core_api):
    _setup_create_job(launcher, mock_batch_api, mock_core_api)
    long_repo = "proj/" + "x" * 100
...
def test_create_job_propagates_launch_limit_error(launcher, mocker):
    mocker.patch.object(launcher, "_enforce_limits", side_effect=LaunchLimitExceededError("at limit"))
    with pytest.raises(LaunchLimitExceededError):
        launcher.create_job(image="img:latest", repo="proj/app", tag="latest")


def test_create_job_idempotent_on_409_service_conflict(launcher, mock_batch_api, mock_core_api):
    _setup_create_job(launcher, mock_batch_api, mock_core_api)
...
def test_create_job_reraises_non_409_api_exception(launcher, mock_batch_api, mock_core_api):
    _setup_create_job(launcher, mock_batch_api, mock_core_api)
...
def _setup_status(mock_batch_api, mock_core_api, job=None, pods=None, svc=None):
    mock_batch_api.list_namespaced_job.return_value = _jobs_list(job) if job else _jobs_list()
    mock_core_api.list_namespaced_pod.return_value = _pod_list(*pods) if pods else _pod_list()
    mock_core_api.list_namespaced_service.return_value = _svc_list(svc) if svc else _svc_list()
...
def test_get_launch_status_cleans_up_service_on_succeeded(launcher, mock_batch_api, mock_core_api):
    job = _make_job(succeeded=1)
    svc = _service("my-svc")
...
def test_get_launch_status_cleans_up_service_on_failed(launcher, mock_batch_api, mock_core_api):
    job = _make_job(failed=1)
    svc = _service("my-svc")
    _setup_status(mock_batch_api, mock_core_api, job=job, svc=svc)
    launcher.get_launch_status(launch_id="test-id")
...
def test_get_launch_status_returns_lb_urls_when_ingress_has_ip(launcher, mock_batch_api, mock_core_api):
    job = _make_job(active=1)
...
def test_get_launch_status_prefers_hostname_over_ip(launcher, mock_batch_api, mock_core_api):
    job = _make_job(active=1)
    ingress_item = MagicMock()
...
def test_get_launch_status_cleans_up_service_when_pod_gone(launcher, mock_batch_api, mock_core_api):
...
def test_get_launch_status_access_pending_when_no_urls(launcher, mock_batch_api, mock_core_api):
    job = _make_job(active=1)
...
def test_get_launch_status_start_time_is_iso_string(launcher, mock_batch_api, mock_core_api):
...
def test_get_launch_status_completion_time_is_iso_string(launcher, mock_batch_api, mock_core_api):
    from datetime import datetime, timezone
    job = _make_job(succeeded=1)
